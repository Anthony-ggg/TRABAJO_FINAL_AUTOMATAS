...
public class QueryExecutorService {
    ...
    public void execute(ASTNode ast, CompilerService.CompileResult result) {
        if (ast == null) return;
        ...
        try {
            ...
            result.agregarError(new com.compilador.shared.model.CompileError(
                    com.compilador.shared.model.CompileError.Fase.EJECUCION, "ERROR_EJECUCION",
    ...
    private void executeSelect(ASTNode ast, CompilerService.CompileResult result) throws Exception {
    ...
    private void executeInsert(ASTNode ast, CompilerService.CompileResult result) throws Exception {
        String tabla = extractTableName(ast);
        if (tabla == null) throw new IllegalArgumentException("No se especifico la tabla");
        if (!tableExists(tabla)) throw new IllegalArgumentException("La tabla '" + tabla + "' no existe");
        ...
        if (valoresStr.size() != columnas.size()) {
            throw new IllegalArgumentException("El numero de valores (" + valoresStr.size()
                    + ") no coincide con el numero de columnas de la tabla (" + columnas.size() + ")");
        }
    ...
    private void executeUpdate(ASTNode ast, CompilerService.CompileResult result) throws Exception {
        String tabla = extractTableName(ast);
        if (tabla == null) throw new IllegalArgumentException("No se especifico la tabla");
        if (!tableExists(tabla)) throw new IllegalArgumentException("La tabla '" + tabla + "' no existe");
    ...
    private void executeDelete(ASTNode ast, CompilerService.CompileResult result) throws Exception {
        String tabla = extractTableName(ast);
        if (tabla == null) throw new IllegalArgumentException("No se especifico la tabla");
        if (!tableExists(tabla)) throw new IllegalArgumentException("La tabla '" + tabla + "' no existe");
